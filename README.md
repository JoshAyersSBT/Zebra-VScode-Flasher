...
On Windows, it bootstraps WSL Debian when needed and creates a default Linux flasher account automatically. The account defaults to username `flasher` and password `flasher`; override these with `zebra.wslFlasherUsername` and `zebra.wslFlasherPassword` before setup if you need different credentials. Zebra configures this managed user for passwordless `sudo` so the native setup does not stop at package-install prompts. If Windows requires a reboot during WSL installation, reboot and run the command again. Once Debian is available, it installs Linux build packages, partially clones MicroPython `v1.28.0`, partially clones ESP-IDF `v5.5.1` with shallow submodules, sparsely checks out only `micropython/cmodules` from the configured Zebra driver repo, installs ESP-IDF tools for `esp32`, builds `mpy-cross`, and prepares ESP32 submodules.
